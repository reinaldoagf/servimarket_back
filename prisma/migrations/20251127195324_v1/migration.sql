...
CREATE TABLE `Product` (
    ...
    `exemptFromVAT` BOOLEAN NOT NULL DEFAULT true,
    `unitMeasurement` ENUM('noaplica', 'kilogramos', 'gramos', 'miligramos', 'kilolitros', 'litros', 'mililitros') NULL DEFAULT 'gramos',
...
CREATE TABLE `Business` (
    ...
    `name` VARCHAR(191) NOT NULL,
    `type` ENUM('minimercado', 'supermercado', 'hypermercado', 'farmacia', 'panaderia', 'otro') NOT NULL DEFAULT 'minimercado',
...
-- CreateTable
CREATE TABLE `PurchaseByCategory` (
    `id` VARCHAR(191) NOT NULL,
...
CREATE TABLE `Purchase` (
    ...
    `productStockId` VARCHAR(191) NULL,
...
-- AddForeignKey
ALTER TABLE `PurchaseByCategory` ADD CONSTRAINT `PurchaseByCategory_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `User`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Purchase` ADD CONSTRAINT `Purchase_businessBranchPurchaseId_fkey` FOREIGN KEY (`businessBranchPurchaseId`) REFERENCES `BusinessBranchPurchase`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Purchase` ADD CONSTRAINT `Purchase_productStockId_fkey` FOREIGN KEY (`productStockId`) REFERENCES `ProductStock`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
